...
import {
  ...
  Command, Copy, CreditCard, Cpu, Database, DollarSign, ExternalLink, FolderGit2, Github, Globe, Home, Landmark, Link2, Mail, Music2,
...
/* ── Types ─────────────────────────────────────────────────────── */
type Tab = "Home" | "Projects" | "Tasks" | "Analytics" | "Swarm" | "Deployments" | "Settings" | "Revenue" | "Bank" | "SEO Report" | "Outreach";
...
export default function Page() {
  ...
  const [liveRevenue, setLiveRevenue] = useState<any>(null);
  ...
  const fetchOutreach = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      if (data.success) setOutreachLeads(data.leads);
    } catch { /* ignore */ }
  }, []);
  ...
  const generateEmail = async (lead: any) => {
    try {
      ...
      const data = await res.json();
      if (data.success) setGeneratedEmail(data);
    } catch { /* ignore */ }
  };

  const fetchAnalytics = useCallback(async () => {
    try {
      const res = await fetch('/api/openclaw/analytics');
      const data = await res.json();
      if (data.success) setLiveAnalytics(data);
    } catch { /* ignore */ }
  }, []);

  const fetchRevenue = useCallback(async () => {
    try {
      const res = await fetch('/api/revenue/ledger');
      const data = await res.json();
      if (data.success) setLiveRevenue(data);
    } catch { /* ignore */ }
  }, []);
  ...
  const fetchStripeBalance = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      if (data.success) setStripeBalance(data);
    } catch { /* ignore */ }
  }, []);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (tab === "Projects" || tab === "Deployments") fetchProjects();
      if (tab === "Analytics" || tab === "Tasks") fetchAnalytics();
      if (tab === "Revenue" || tab === "Bank") { fetchRevenue(); fetchStripeBalance(); }
      if (tab === "Outreach") fetchOutreach();
      if (tab === "Home") { fetchAnalytics(); fetchRevenue(); fetchStripeBalance(); }
    };
    ...
    return () => clearInterval(id);
  }, [settings.openClawEnabled, settings.openClawHeartbeatSec, checkGateway, tab, fetchSwarmData, fetchProjects, fetchAnalytics, fetchRevenue, fetchOutreach, fetchStripeBalance]);
  ...
  const cmdkItems = [
    ...
    { label: "Outreach", action: () => { setTab("Outreach"); setCmdkOpen(false); } },
  ...
  const sidebarItems: { label: Tab; icon: React.ReactNode }[] = [
    ...
    { label: "Outreach", icon: <Mail size={16} /> },
          ...
          <main className="relative min-h-[600px]">
            ...
            {tab === "Home" && (
              <section className="grid grid-cols-1 xl:grid-cols-4 gap-4">
                {/* Live Stats Row */}
                {[
                  { label: "Projects", value: codingAiProjects.length || '—', icon: "📁", color: "text-indigo-400" },
                  { label: "Leads Captured", value: liveRevenue?.totalLeads ?? '—', icon: "🎯", color: "text-amber-400" },
                  { label: "Tasks Done", value: liveAnalytics?.statusCounts?.succeeded ?? '—', icon: "✅", color: "text-emerald-400" },
                  { label: "Active Agents", value: liveAnalytics ? Object.values(liveAnalytics.agentStatus ?? {}).filter((s: any) => s !== 'idle').length : '—', icon: "🤖", color: "text-violet-400" },
                ].map(stat => (
                  <div key={stat.label} className="rounded-2xl border border-white/5 bg-slate-900/40 p-4 flex items-center gap-4">
                    <span className="text-3xl">{stat.icon}</span>
                    <div>
                      <p className={cx("text-2xl font-black font-mono", stat.color)}>{stat.value}</p>
                      <p className="text-[9px] uppercase tracking-widest text-slate-500 font-bold mt-0.5">{stat.label}</p>
                    </div>
                  </div>
                ))}
              ...
              <section className="space-y-8">
                ...
                {codingAiProjects.length > 0 ? (
                  <div className="flex flex-col gap-12">
                    {Object.entries(
                      codingAiProjects.reduce((acc: Record<string, any[]>, proj: any) => {
                        const cat = proj.category || 'Other';
                        if (!acc[cat]) acc[cat] = [];
                        acc[cat].push(proj);
                        return acc;
                      }, {})
                    ).sort(([a], [b]) => a.localeCompare(b)).map(([categoryName, projects]: [string, any]) => (
                      <div key={categoryName}>
                        <h3 className="text-sm font-bold font-mono text-slate-300 mb-6 uppercase tracking-widest flex items-center gap-3">
                          <span className="w-8 h-[1px] bg-indigo-500/50"></span>
                          {categoryName}
                          <span className="flex-1 h-[1px] bg-white/5"></span>
                          <span className="text-[10px] text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full">{projects.length}</span>
                        </h3>
                      ...
                      return (
                        <motion.div 
                          ...
                          onClick={() => executeProjectAction(proj, 'open_folder')}
                          className={cx(
                            "group/proj relative overflow-hidden rounded-2xl border p-4 transition-all duration-300 hover:scale-[1.02] cursor-pointer",
                            glowColor
                          )}
                        >
                          {/* Top Row: Name & Status */}
                          ...
                          <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm opacity-0 group-hover/proj:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3 p-4">
                             <button onClick={(e) => { e.stopPropagation(); executeProjectAction(proj, 'terminal_open'); }} className="flex-1 max-w-[100px] bg-slate-800 hover:bg-slate-700 text-white text-[9px] font-black uppercase tracking-widest py-2 rounded-lg border border-white/10 transition-colors shadow-lg">
                               Terminal
                             </button>
                             {proj.isNode && (
                               <button onClick={(e) => { e.stopPropagation(); executeProjectAction(proj, 'npm_dev'); }} className="flex-1 max-w-[100px] bg-indigo-500 hover:bg-indigo-400 text-white text-[9px] font-black uppercase tracking-widest py-2 rounded-lg border border-indigo-400/50 shadow-[0_0_15px_rgba(99,102,241,0.5)] transition-colors">
                                 Run Dev
                               </button>
                             )}
                          </div>
            ...
            {tab === "Tasks" && (
              <section className="space-y-4">
                ...
                {/* Live OpenClaw Subagent Queue */}
                <GlassCard title="Live Subagent Queue" icon={<Rocket size={16} className="text-indigo-400" />}>
                  <div className="space-y-2 max-h-64 overflow-auto">
                    {liveAnalytics?.tasks?.length > 0 ? liveAnalytics.tasks.map((t: any) => {
                      const statusColor = t.status === 'succeeded' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20'
                        : t.status === 'running' ? 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20 animate-pulse'
                        : t.status === 'failed' ? 'text-rose-400 bg-rose-500/10 border-rose-500/20'
                        : 'text-slate-400 bg-slate-800 border-white/5';
                      const label = (t.task || '').split('\n')[0].slice(0, 72);
                      return (
                        <div key={t.task_id} className="flex items-start gap-3 rounded-lg bg-slate-800/40 border border-white/5 p-2.5">
                          <span className={cx("shrink-0 text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded border", statusColor)}>{t.status}</span>
                          <div className="min-w-0">
                            <p className="text-xs text-slate-300 font-medium truncate">{label}</p>
                            {t.progress_summary && <p className="text-[10px] text-slate-500 mt-0.5 truncate">{t.progress_summary}</p>}
                          </div>
                        </div>
                      );
                    }) : (
                      <p className="text-xs text-slate-500 text-center py-6">Queue is clear — all tasks completed.</p>
                    )}
                  </div>
                </GlassCard>
              </section>
            ...
            {tab === "Analytics" && (
              <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {/* Live KPI Stats Row */}
                {[
                  { label: "Tasks Completed", value: liveAnalytics?.statusCounts?.succeeded ?? '—', color: "text-emerald-400" },
                  { label: "Completion Rate", value: liveAnalytics ? `${liveAnalytics.completionRate}%` : '—', color: "text-indigo-400" },
                  { label: "Total Leads", value: liveAnalytics?.totalLeads ?? '—', color: "text-amber-400" },
                ].map(kpi => (
                  <GlassCard key={kpi.label} title={kpi.label} icon={<TrendingUp size={16} />}>
                    <p className={cx("text-4xl font-black font-mono mt-2", kpi.color)}>{kpi.value}</p>
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest mt-1 font-bold">Live from OpenClaw DB</p>
                  </GlassCard>
                ))}

                {/* Agent XP Leaderboard */}
                <GlassCard title="Agent Leaderboard" icon={<ShieldCheck size={16} />} span="lg:col-span-2">
                  <div className="space-y-2">
                    {liveAnalytics?.agentXP ? Object.entries(liveAnalytics.agentXP).sort(([,a]: any, [,b]: any) => b.xp - a.xp).map(([name, data]: [string, any]) => (
                      <div key={name} className="flex items-center gap-3">
                        <span className="text-[10px] font-black uppercase w-20 text-slate-400 shrink-0">{name}</span>
                        <div className="flex-1 h-2 rounded-full bg-slate-800 overflow-hidden">
                          <div className="h-full rounded-full bg-gradient-to-r from-violet-500 to-indigo-400 transition-all duration-700" style={{ width: `${Math.min((data.xp % 100), 100)}%` }} />
                        </div>
                        <span className="text-[10px] font-mono text-slate-400 w-16 text-right shrink-0">Lv{data.level} · {data.xp}xp</span>
                      </div>
                    )) : <p className="text-xs text-slate-500 text-center py-4">Loading agent data...</p>}
                  </div>
                </GlassCard>

                {/* System Watchdog */}
                <GlassCard title="Watchdog" icon={<ShieldCheck size={16} />}>
                ...
                {/* Live OpenClaw Log Feed */}
                <GlassCard title="Live Agent Log Feed" icon={<Activity size={16} />} span="lg:col-span-3">
                  <div className="max-h-56 overflow-auto space-y-1.5 text-xs">
                    {liveAnalytics?.logs?.length > 0 ? [...liveAnalytics.logs].reverse().map((log: any, i: number) => (
                      <div key={i} className="rounded-lg bg-slate-800/60 border border-white/5 p-2 flex items-start gap-3">
                        <span className="text-[8px] font-black uppercase tracking-widest px-1.5 py-1 rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 shrink-0 mt-0.5">{log.agent}</span>
                        <span className="text-slate-300">{log.message}</span>
                        <span className="text-slate-600 font-mono shrink-0 ml-auto">{new Date(log.timestamp).toLocaleTimeString()}</span>
                      </div>
                    )) : <p className="text-slate-500 text-center py-4">No events yet. Interact with the dashboard to generate logs.</p>}
                  </div>
                </GlassCard>
              </section>
            )}
              ...
              <section className="space-y-8">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex bg-emerald-500/10 border border-emerald-500/20 px-4 py-2 rounded-xl text-emerald-400 font-mono text-xl font-black shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                    £{(liveRevenue?.totalRevenue ?? 0).toFixed(2)} <span className="text-[10px] uppercase text-emerald-500/50 self-end ml-2 mb-1">GBP</span>
                  </div>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  {/* Daily Revenue Bar Chart */}
                  <GlassCard title="Daily Lead Value" icon={<TrendingUp size={16} className="text-emerald-400" />} span="lg:col-span-2">
                    <div className="flex items-end gap-2 h-36 mt-2">
                      {(liveRevenue?.dailyRevenue || []).map((d: any, i: number) => {
                        const max = Math.max(...(liveRevenue?.dailyRevenue || [{ value: 1 }]).map((x: any) => x.value), 1);
                        const pct = Math.round((d.value / max) * 100);
                        return (
                          <div key={d.label} className="flex-1 flex flex-col items-center gap-1">
                            <span className="text-[9px] font-mono text-emerald-400">£{d.value}</span>
                            <div className="w-full rounded-t-lg bg-gradient-to-t from-emerald-600 to-emerald-400 transition-all duration-700"
                              style={{ height: chartVisible ? `${pct}%` : '0%', transitionDelay: `${i * 80}ms` }} />
                  ...
                  <GlassCard title="Agent Output" icon={<Activity size={16} className="text-emerald-400" />}>
                    <div className="space-y-2">
                      {(liveRevenue?.agentRevenue || []).map((a: any) => (
                        <div key={a.name} className="flex items-center justify-between p-2 rounded-lg bg-emerald-950/20 border border-emerald-500/10">
                          <div>
                            <p className="text-[10px] font-black uppercase text-slate-300">{a.name}</p>
                            <p className="text-[9px] text-slate-500">{a.tasksCompleted} tasks · Lv{a.level}</p>
                          </div>
                          <span className="text-xs font-mono font-bold text-emerald-400">£{a.estimatedRevenue}</span>
                        </div>
                      ))}
                    </div>
                  </GlassCard>
                  ...
                  <GlassCard title="Payment Infrastructure" icon={<CreditCard size={16} className="text-emerald-400" />} span="lg:col-span-3">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                      {[{ icon: <Link2 size={14} />, label: "Stripe API Access", sub: "Live Mode Secret Key", status: "CONNECTED" },
                        { icon: <Activity size={14} />, label: "Webhook Sync", sub: "Event Listener", status: "ACTIVE" },
                        { icon: <ShieldCheck size={14} />, label: `${liveRevenue?.totalLeads ?? 0} Leads Captured`, sub: `${liveRevenue?.highValueLeads ?? 0} high-value (score ≥80)`, status: "LIVE" },
                      ].map(item => (
                        <div key={item.label} className="flex items-center justify-between p-3 rounded-lg bg-emerald-950/30 border border-emerald-500/10">
                          <div className="flex items-center gap-3">
                            <div className="h-8 w-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-400">{item.icon}</div>
                            <div><p className="text-xs font-bold text-slate-200">{item.label}</p><p className="text-[9px] uppercase tracking-widest text-slate-500 mt-0.5">{item.sub}</p></div>
                          </div>
                          <span className="px-2 py-1 text-[8px] font-black uppercase tracking-widest bg-emerald-500/10 text-emerald-400 rounded">{item.status}</span>
                        </div>
                      ))}
                    </div>
                  </GlassCard>
                  ...
                  {/* PayPal Payment Hub */}
                  <GlassCard title="Payment Links" icon={<Link2 size={16} className="text-emerald-400" />} span="lg:col-span-3">
                    <div className="space-y-3">
                      {[
                        { name: 'Website Refresh & Optimisation', price: '£300', link: 'https://www.paypal.com/ncp/payment/SL5S9DW2G6KXG', score: 'Score < 60', color: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
                        { name: 'Website Design & Development', price: '£500', link: 'https://www.paypal.com/ncp/payment/A2KVSJ2XWG6JG', score: 'Score 60–79', color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
                        { name: 'Premium Build & Digital Strategy', price: '£800', link: 'https://www.paypal.com/ncp/payment/CA33ZAJ79BDSY', score: 'Score 80+', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' },
                      ].map(tier => (
                        <div key={tier.price} className="flex items-center gap-4 p-3 rounded-xl bg-slate-800/40 border border-white/5 hover:border-emerald-500/20 transition-all group">
                          <div className="shrink-0 h-10 w-10 rounded-xl bg-[#0070BA]/20 flex items-center justify-center text-lg font-black text-[#0070BA]">P</div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <p className="text-xs font-bold text-slate-200">{tier.name}</p>
                              <span className="text-lg font-black font-mono text-emerald-400">{tier.price}</span>
                            </div>
                            <p className="text-[10px] text-slate-500 font-mono truncate mt-0.5">{tier.link}</p>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <span className={cx("px-2 py-1 text-[7px] font-black uppercase tracking-widest rounded border", tier.color)}>{tier.score}</span>
                            <span className="px-2 py-1 text-[8px] font-black uppercase tracking-widest bg-emerald-500/10 text-emerald-400 rounded border border-emerald-500/20">LIVE</span>
                            <button
                              onClick={() => { navigator.clipboard.writeText(tier.link); toast(`${tier.price} link copied!`, 'success'); }}
                              className="px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[9px] font-black uppercase hover:bg-emerald-500/20 transition-colors opacity-0 group-hover:opacity-100"
                            >
                              <Copy size={12} />
                            </button>
                          </div>
                      ...
                      <div className="flex gap-3 mt-2">
                        <button
                          onClick={() => { setTab("Outreach" as Tab); toast('Outreach emails auto-select the right tier based on lead score', 'info'); }}
                          className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-500/10 text-rose-400 border border-rose-500/20 font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-rose-500/20 transition-colors"
                        >
                          <Mail size={14} /> Send to Leads (Auto-Tiered)
                        </button>
                      </div>

                      <p className="text-[9px] text-slate-500 text-center uppercase tracking-widest font-bold">Outreach emails auto-select tier by lead score · PayPal balance → bank in 1-2 days</p>
                    </div>
                  </GlassCard>
                </div>
              </section>
              ...
              <section className="space-y-8">
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                   <div className="lg:col-span-2">
                     <GlassCard title="" glowColor="rgba(245,158,11,0.1)" className="h-full flex flex-col justify-center items-center py-16 relative overflow-hidden">
                        ...
                        <h1 className="text-6xl md:text-8xl font-black font-mono tracking-tighter text-amber-400 drop-shadow-[0_0_20px_rgba(245,158,11,0.3)]">
                          £{Math.floor(stripeBalance?.available ?? 0)}<span className="text-2xl md:text-4xl text-amber-500/50">.{String(Math.round(((stripeBalance?.available ?? 0) % 1) * 100)).padStart(2, '0')}</span>
                        </h1>
                        {stripeBalance?.source === 'simulated' && (
                          <p className="text-xs text-rose-400 mt-2 font-bold bg-rose-500/10 px-3 py-1 rounded">Showing simulated zero balance until real Stripe key is added to .env.local</p>
                        )}
                        <div className="mt-12 flex flex-col items-center z-10 w-full max-w-md">
                           <div className="mb-8 w-full p-4 rounded-xl bg-amber-950/30 border border-amber-500/20 flex flex-col items-center">
                              ...
                              <p className="text-[10px] text-center text-slate-500 mt-2 leading-relaxed">
                                All Stripe Payouts route directly to <span className="text-amber-400 font-mono">moazzinzaman&apos;s</span> primary bank account. Agents do NOT take a commission.
                              </p>
                           </div>
                           <div className="flex gap-4 w-full px-8">
                             <button 
                               onClick={async () => {
                                 toast('Initiating withdrawal to your bank account...', 'info');
                                 try {
                                   const res = await fetch('/api/stripe/withdraw', { method: 'POST' });
                                   const data = await res.json();
                                   if (data.success) {
                                     toast(`✅ Withdrawal successful! Arriving: ${new Date(data.expectedArrival).toLocaleDateString()}`, 'success');
                                     fetchStripeBalance();
                                   } else {
                                     toast(data.error || 'Withdrawal failed', 'error');
                                   }
                                 } catch {
                                   toast('Network error during withdrawal', 'error');
                                 }
                               }}
                               className="flex-1 flex items-center justify-center gap-2 px-8 py-3 bg-amber-500 text-black font-black uppercase tracking-widest text-[10px] rounded-xl shadow-[0_0_20px_rgba(245,158,11,0.4)] hover:scale-[1.02] transition-transform">
                                Withdraw Funds
                             </button>
                             <button className="flex-1 flex items-center justify-center gap-2 px-8 py-3 bg-amber-500/10 text-amber-400 border border-amber-500/30 font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-amber-500/20 transition-colors">
                                Reinvest
                             </button>
                           </div>
                        </div>
                   ...
                   <div className="space-y-6">
                     <GlassCard title="Live Lead Ledger" icon={<Activity size={16} className="text-amber-400" />}>
                       <div className="space-y-2 max-h-80 overflow-auto">
                         {liveRevenue?.leads?.length > 0 ? liveRevenue.leads.map((lead: any, i: number) => (
                           <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-amber-950/20 border border-amber-500/10">
                             <div>
                               <p className="text-[10px] font-bold text-slate-300">{lead.company}</p>
                               <p className="text-[9px] text-slate-500">{lead.name} · Score: {lead.score}</p>
                             </div>
                             <span className={cx("text-[9px] font-mono font-bold", lead.score >= 80 ? "text-emerald-400" : "text-slate-400")}>£5.00</span>
                           </div>
                         )) : (
            ...
            {tab === "Outreach" && (
              <section className="space-y-8">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h2 className="text-3xl font-black tracking-tighter uppercase italic bg-gradient-to-r from-pink-400 to-rose-600 bg-clip-text text-transparent">Outreach Hub</h2>
                    <p className="text-[10px] text-rose-500/80 uppercase tracking-[0.4em] font-bold">Lead Engagement & Email Generator</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="flex bg-rose-500/10 border border-rose-500/20 px-4 py-2 rounded-xl text-rose-400 font-mono text-xl font-black shadow-[0_0_15px_rgba(244,63,94,0.2)] items-center gap-2">
                      <Mail size={18} /> {outreachLeads.length} LEADS
                    </div>
                    <button onClick={fetchOutreach} className="flex items-center gap-2 rounded-xl bg-rose-500/10 border border-rose-400/20 px-5 py-2.5 text-xs font-bold hover:bg-rose-500/20 text-rose-300 transition-all active:scale-95">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6">
                  {/* Lead Table */}
                  <GlassCard title="Lead Database" icon={<Database size={16} className="text-rose-400" />}>
                    <div className="space-y-2 max-h-[500px] overflow-auto">
                      {outreachLeads.length > 0 ? outreachLeads.map((lead: any, i: number) => (
                        <motion.div
                          key={lead.id || i}
                          initial={{ opacity: 0, x: -10 }}
                          animate={{ opacity: 1, x: 0 }}
                          transition={{ delay: i * 0.03 }}
                          className="flex items-center justify-between p-3 rounded-xl bg-slate-800/40 border border-white/5 hover:border-rose-500/20 transition-all group"
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <div className={cx(
                              "shrink-0 h-8 w-8 rounded-full flex items-center justify-center text-[10px] font-black",
                              lead.score >= 80 ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" :
                              lead.score >= 60 ? "bg-amber-500/20 text-amber-400 border border-amber-500/30" :
                              "bg-slate-700 text-slate-400 border border-white/10"
                            )}>
                              {lead.score}
                            </div>
                            <div className="min-w-0">
                              <p className="text-xs font-bold text-slate-200 truncate">{lead.company}</p>
                              <p className="text-[10px] text-slate-500 truncate">{lead.name} · {lead.email}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onClick={() => generateEmail(lead)}
                              className="px-3 py-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-400 text-[9px] font-black uppercase tracking-wider hover:bg-rose-500/20 transition-colors"
                            >
                              ✉ Generate Email
                            </button>
                          </div>
                        ...
                        <div className="py-16 text-center">
                          <Mail size={48} className="mx-auto text-slate-800 mb-4" />
                          <p className="text-sm text-slate-500">No leads loaded. Click Refresh to pull from OpenClaw memory.</p>
                        </div>
                      )}
                    </div>
                  </GlassCard>
                  ...
                  <div className="space-y-4">
                    <GlassCard title="Email Preview" icon={<Send size={16} className="text-rose-400" />}>
                      {generatedEmail ? (
                        <div className="space-y-4">
                          <div>
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1">To</p>
                            <p className="text-xs font-mono text-slate-300">{generatedEmail.lead?.email}</p>
                          </div>
                          <div>
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1">Subject</p>
                            <p className="text-xs font-bold text-slate-200">{generatedEmail.subject}</p>
                          </div>
                          <div>
                            <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-1">Body</p>
                            <pre className="text-[11px] text-slate-300 whitespace-pre-wrap font-sans leading-relaxed bg-black/30 rounded-lg p-3 border border-white/5 max-h-64 overflow-auto">{generatedEmail.body}</pre>
                          </div>
                          <div className="flex gap-2">
                            <button
                              onClick={() => { navigator.clipboard.writeText(`Subject: ${generatedEmail.subject}\n\n${generatedEmail.body}`); toast("Email copied to clipboard!", "success"); }}
                              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-rose-500 text-white font-black uppercase tracking-widest text-[10px] rounded-xl hover:bg-rose-400 transition-colors"
                            >
                            ...
                            <button
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  const data = await res.json();
                                  if (data.success) {
                                    toast(`✅ Email sent to ${generatedEmail.lead?.email}!`, 'success');
                                  } else {
                                    toast(data.error || 'Failed to send', 'error');
                                  }
                                } catch { toast('Network error', 'error'); }
                              }}
                              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-rose-500 to-pink-500 text-white font-black uppercase tracking-widest text-[10px] rounded-xl hover:from-rose-400 hover:to-pink-400 transition-colors shadow-[0_0_15px_rgba(244,63,94,0.3)]"
                            >
                              <Send size={12} /> Send from Summit Forge
                            </button>
                          </div>
                        </div>
                      ) : (
                        <div className="py-16 text-center">
                          <Send size={32} className="mx-auto text-slate-800 mb-4" />
                          <p className="text-xs text-slate-500">Click "Generate Email" on any lead to preview a personalised outreach email.</p>
                        </div>
                      )}
                    </GlassCard>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="rounded-xl border border-white/5 bg-slate-900/40 p-3 text-center">
                        <p className="text-2xl font-black font-mono text-emerald-400">{outreachLeads.filter((l: any) => l.score >= 80).length}</p>
                        <p className="text-[8px] uppercase tracking-widest text-slate-500 font-bold mt-1">High Value</p>
                      </div>
                      <div className="rounded-xl border border-white/5 bg-slate-900/40 p-3 text-center">
                        <p className="text-2xl font-black font-mono text-amber-400">{outreachLeads.filter((l: any) => l.score >= 60 && l.score < 80).length}</p>
                        <p className="text-[8px] uppercase tracking-widest text-slate-500 font-bold mt-1">Medium Value</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <section className="space-y-8">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex bg-cyan-500/10 border border-cyan-500/20 px-4 py-2 rounded-xl text-cyan-400 font-mono text-xl font-black shadow-[0_0_15px_rgba(6,182,212,0.2)] items-center gap-2">
                    ...
                    {codingAiProjects.filter((p: any) => p.status === 'ONLINE' || p.status === 'READY').length} ONLINE
                  </div>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {codingAiProjects.filter((p: any) => p.status === 'ONLINE' || p.status === 'READY').map((dep: any, idx: number) => (
                    <GlassCard key={dep.id} title={dep.name} icon={<Server className="text-cyan-400" size={16} />} glowColor="rgba(6,182,212,0.1)">
                      <div className="space-y-5">
                        ...
                        <div className="flex justify-between items-start">
                           <a href={`http://localhost:${3000 + (idx % 1000)}`} target="_blank" rel="noopener noreferrer" className="group flex flex-col cursor-pointer">
                              <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1 flex items-center gap-1 group-hover:text-cyan-400 transition-colors">
                                Local URL <ExternalLink size={10} />
                              </span>
                              <span className="text-sm font-mono text-slate-300 group-hover:text-white transition-colors border-b border-transparent group-hover:border-white/20 pb-0.5">{`localhost:${3000 + (idx % 1000)}`}</span>
                           </a>
                        ...
                        <div className="grid grid-cols-3 gap-2 py-3 border-y border-white/5">
                           <div className="text-center">
                              ...
                              <p className="text-xs font-black text-slate-200 mt-1">Local Dev</p>
                           </div>
                           <div className="text-center border-x border-white/5">
                              <p className="text-[9px] uppercase tracking-widest font-bold text-slate-500">Stack</p>
                              <p className="text-[10px] font-mono text-cyan-400 mt-1">{dep.stack[0]}</p>
                           </div>
                           <div className="text-center">
                              <p className="text-[9px] uppercase tracking-widest font-bold text-slate-500">Weight</p>
                              <p className="text-xs font-mono text-slate-300 mt-1">{(dep.size / (1024 * 1024)).toFixed(1)} MB</p>
                           </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-1 text-[10px] font-black uppercase tracking-widest">
                           <span className="text-slate-500 flex items-center gap-1">
                             <Clock3 size={10} /> {new Date(dep.lastModified).toLocaleDateString()}
                           </span>
                           <div className="flex gap-2">
                             <button onClick={() => executeProjectAction(dep, 'terminal_open')} className="px-3 py-1.5 bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 rounded-lg transition-colors border border-white/5">VS Code</button>
                             <button onClick={() => executeProjectAction(dep, 'npm_dev')} className="px-3 py-1.5 bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 rounded-lg hover:bg-cyan-500/20 transition-colors">Start Server</button>
                           </div>
                        </div>
